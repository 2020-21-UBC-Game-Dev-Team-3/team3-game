...
public class PlayerActionController : MonoBehaviour, IDamageable
{
    ...
    private bool inVent;
    [HideInInspector] public bool hasVotingCooldownRunning;
    ...
    [HideInInspector] public float votingCooldownTimer;
    public float votingCooldown = 20f;
    ...
    void EmergencyButtonCooldown()
    {
        votingCooldownTimer -= Time.deltaTime;
        if (votingCooldownTimer <= 0)
        {
            hasVotingCooldownRunning = false;
    ...
    void ChooseInteractionEvent(Interactable interactable)
    {
        switch (interactable.interactableName)
        {
            ...
            case "Emergency button":
                if (!hasVotingCooldownRunning && emergencyMeetingCount < 2)
                {
                    emergencyMeetingCount++;
